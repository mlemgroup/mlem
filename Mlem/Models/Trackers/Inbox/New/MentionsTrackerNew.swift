...
class MentionsTrackerNew: ObservableObject, InboxFeedSubTracker {
    // @Dependency(\.mentionsRepository) var mentionsRepository
    ...
    func nextItemSortVal(sortType: InboxSortType) async throws -> InboxSortVal? {
        ...
        if cursor < mentions.count {
            ...
            return cursor < mentions.count ? mentions[cursor].getInboxSortVal(sortType: sortType) : nil
        }
    }

    func consumeNextItem() -> InboxItemNew? {
        assert(cursor < mentions.count, "consumeNextItem called on a tracker without a next item!")
    ...
    private func loadPage(_ pageToLoad: Int, clearBeforeReset: Bool = false) async throws {
        ...
        print("attempting to load page \(pageToLoad) of mentions")
        ...
        guard pageToLoad == page + 1 else {
            print("will not load page \(pageToLoad) of mentions (have loaded \(page) pages)")
            return
        }
        
//        let newMentions = try await messageRepository.loadMessages(
//            page: pageToLoad,
//            limit: internetSpeed.pageSize,
//            unreadOnly: unreadOnly
//        )
        let newMentions: [MentionModel] = .init()
        page = pageToLoad
        
        // if no messages show up and no error was thrown, there's nothing left to load
        if newMentions.isEmpty {
            print("received no mentions!")
            loadingState = .done
            return
        }
        ...
        let allowedMentions = storeIdsAndDedupe(newMentions: newMentions)
        
        // if loading page 1, we can just do a straight assignment regardless of whether we did clearBeforeReset
        if page == 1 {
            mentions = allowedMentions
        } else {
            add(toAdd: allowedMentions)
        }
    
        loadingState = .idle
    }

    private func add(toAdd: [MentionModel]) {
        // TODO: filtering
        mentions.append(contentsOf: toAdd)
    }
    ...
    private func storeIdsAndDedupe(newMentions: [MentionModel]) -> [MentionModel] {
        let accepted = newMentions.filter { ids.insert($0.uid).inserted }
        return accepted
    }
}
