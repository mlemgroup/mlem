...
struct FullyQualifiedLabelView: View {
    ...
    var body: some View {
        HStack(spacing: 7) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                .imageScale(.small)
                .offset(y: 1)
                if let note = (entity as? any DeprecatedPerson)?.note, feedContext != .person {
                    self.note(text: note)
                }
            }
        }
    ...
    var flairs: [PersonFlair] {
        guard showFlairs, let person = entity as? any DeprecatedPerson else { return [] }
    ...
    var interactableContext: (any InteractableProviding)? {
        guard let person = entity as? any DeprecatedPerson else { return nil }
    ...
    var accessibilityLabel: String {
        ...
        if let note = (entity as? any DeprecatedPerson)?.note {
